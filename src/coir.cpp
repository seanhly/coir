...
Context context_from_sorted_candidates(Candidate *candidates_ptr, ui64 len) {
	Context ctx;
	...
	for (ui64 i = 0; i < cutoff; i++)
		dcg += discounted_gain(candidates_ptr[i].relevance, i + 1);
	ctx.min_dcg = QUALITY_THRESHOLD * dcg;
	ctx.rank_contexts =
	...
	for (int j = 1; j <= PAGE_LENGTH; j++) {
		...
		if (
			...
			ctx.rank_contexts[rank_contexts_c].candidates_ptr =
				(Candidate**) malloc((i - offset) * sizeof(Candidate*));
...
void update_relevance(FILE *job_fp, FILE *job_end_fp) {
	...
	while (fread(&doc_id, sizeof(ui64), 1, job_fp) == 1) {
		if (fread(&relevance, sizeof(f64), 1, job_fp) == 1) {
			...
			row++;
		} else err_exit("error reading relevance\n");
	}
	...
	for (int i = 0; i < row; i++) {
		safe_write_item(&doc_relevance_update[i].doc_id, sizeof(ui64), qrel_fp);
		safe_write_item(&doc_relevance_update[i].relevance, sizeof(f64), qrel_fp);
		safe_write_item(&doc_relevance_update[i].exposure, sizeof(ui64), qrel_fp);
	}
	fclose(qrel_fp);
	Context ctx = context_from_sorted_candidates(doc_relevance_update, row);
...
void gen_ranking(FILE *job_fp, FILE *job_end_fp) {
	...
	if (it == CANDIDATES.end()) {
		...
		fclose(qrel_fp);
		ctx = context_from_sorted_candidates(candidates_ptr, len);
		CANDIDATES[query_id] = ctx;
	} else
		ctx = it->second;
	...
	std::sort(
		ranking, ranking + local_page_length,
		compare_candidate_ptr
	);
...
void show_candidates(char *input_buffer) {
	char c;
	ui64 offset = 0;
	ui64 job_id = t();
	while ((c = getchar()) != EOF) {
...
int main(int argc, char *argv[]) {
	...
	if (subcommand == RERANK) {
		if (extra_args || !input_pipe_open)
			print_subcommand_help_then_halt(RERANK, 1);
		rerank(input_buffer);
	} else if (subcommand == SHOW_CANDIDATES) {
		if (extra_args || !input_pipe_open)
			print_subcommand_help_then_halt(RERANK, 1);
		show_candidates(input_buffer);
